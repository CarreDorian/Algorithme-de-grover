...
class my_grover():
    ...
    def __init__(self, df, nbr_shots = 50, max_parallel = 10):
        self.df = df
    ...
    def __save_mesure(self, qc, name_file = "histogramme_mesure.png", title = "Distribution des mesures en %"):
        qc.measure_all()
        simulator = AerSimulator(max_parallel_threads=self.max_parallel, max_parallel_experiments=self.max_parallel) # limiter le nombre de thread évite d'exploser la ram utilisé
        compiled_circuit = transpile(qc, simulator)
        result = simulator.run(compiled_circuit, shots=self.nbr_shots).result().get_counts()
